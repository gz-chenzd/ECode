...
{
    public class RedisClientManager : IRedisClientManager, IDisposable
    {
        ...
        class ConnectionPoolConfig
        {
            ...
            public static ConnectionPoolConfig Parse(string config)
            {
                ...
                foreach (var keyValuePair in config.Split(';', StringSplitOptions.RemoveEmptyEntries))
                {
                    ...
                    switch (key)
                    {
                        case "server":
                            ...
                            if (hostPort.Length == 2)
                            {
                                ...
                                if (resolvedValue < 1 || resolvedValue > 65535)
                                {
                        ...
                        case "database":
                            ...
                            {
                                throw new ConfigurationException("Config error: cannot parse 'Database' to integer value.");
                            }

                            if (resolvedValue < 0)
                            {
                                throw new ConfigurationException("Config error: 'Database' must be >= 0.");
                            }

                            result.Database = resolvedValue;
                            break;

                        case "password":
                            if (string.IsNullOrWhiteSpace(value))
                            {
                                throw new ConfigurationException("Config error: 'Password' cannot be empty string.");
                            }
        ...
        private Dictionary<string, ConnectionPool<RedisClient>>         m_pConnectionPools      = null;


        public RedisClientManager(string configKey)
        {
        ...
        public RedisClientManager(string configKey, IShardStrategy shardStrategy)
        {
            AssertUtil.ArgumentNotEmpty(configKey, nameof(configKey));
        ...
        private void ReloadConnectionPools(Dictionary<string, ConnectionPoolConfig> poolsConfig)
        {
            if (m_pPoolConfigs == null)
            {
                ...
                m_pConnectionPools = new Dictionary<string, ConnectionPool<RedisClient>>();
            }

            var newConnectionPools = new Dictionary<string, ConnectionPool<RedisClient>>();
            ...
            foreach (var poolConfig in poolsConfig.Values)
            {
                ...
                var newConnectionPool = new ConnectionPool<RedisClient>(poolConfig.Host, poolConfig.Port, poolConfig.Ssl);
        ...
        public RedisClient GetClient(object shardObject, bool writable)
        {
            var tuple = GetConnectionPool(shardObject, writable);
            var connector = tuple.Item2.GetConnector();

            if (!connector.IsAuthenticated && !string.IsNullOrWhiteSpace(tuple.Item1.Password))
            {
                connector.Auth(tuple.Item1.Password);
            }

            if (tuple.Item1.Database >= 0 && connector.CurrentDatabase != tuple.Item1.Database)
            {
                connector.Select(tuple.Item1.Database);
            }
        ...
        [MethodImpl(MethodImplOptions.Synchronized)]
        private Tuple<ConnectionPoolConfig, ConnectionPool<RedisClient>> GetConnectionPool(object shardObject, bool writable)
        {
            ...
            return new Tuple<ConnectionPoolConfig, ConnectionPool<RedisClient>>(m_pPoolConfigs[shardNo], m_pConnectionPools[shardNo]);
        }
    }
}
