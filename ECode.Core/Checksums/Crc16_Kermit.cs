...
{
    public sealed class Crc16_Kermit : IChecksum
    {
        static readonly ushort[]    CrcTable    = new ushort[] {
        ...
        public void Update(byte[] bytes, int index, int count)
        {
            ...
            var temp = checksum >> 8 | checksum << 8;

            while (--count >= 0)
            {
                temp = CrcTable[(temp ^ bytes[index++]) & 0xFF] ^ (temp >> 8);
            }

            checksum = (ushort)(temp << 8 | temp >> 8);
        }
